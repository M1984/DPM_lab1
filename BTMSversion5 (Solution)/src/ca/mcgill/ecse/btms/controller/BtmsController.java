...
public class BtmsController {
	...
	public static void deleteDriver(int id) throws InvalidInputException {
	...
	public static void deleteRoute(int number) throws InvalidInputException {
	...
	public static void updateRoute(int number, int averageMinutesToNextBusStop, int numberOfBusStops) throws InvalidInputException {
		String error = "";
		Route route = Route.getWithNumber(number);
		if (route == null) {
			error = "A route with this number does not exist. ";
		}
		if (averageMinutesToNextBusStop <= 0) {
			error = error + "The average number of minutes to next bus stop must be greater than zero. ";
		}
		...
		route.setAverageMinutesToNextBusStop(averageMinutesToNextBusStop);
		while (route.hasBusStops()) {
			route.getBusStop(0).delete();
		}
		for (int i=0; i<numberOfBusStops; i++) {
			route.addBusStop(i+1);
		}
	...
	public static void deleteBus(String licencePlate) throws InvalidInputException {
	...
	public static void deleteAssignment(String licencePlate, int number, Date date) throws InvalidInputException {
	...
	public static void deleteSchedule(int driverID, Date date, int number, String licencePlate, String shiftName) throws InvalidInputException {
		RouteAssignment assignment = findRouteAssignment(date, number, licencePlate);
		Shift shift = null;
		try {
			shift = Shift.valueOf(shiftName);
		...
		DriverSchedule schedule = findSchedule(driverID, assignment, shift);
		if (schedule != null) {
			schedule.delete();
			try {
				BtmsPersistence.save(BtmsApplication.getBtms());
	...
	private static DriverSchedule findSchedule(int driverID, RouteAssignment routeAssignment, Shift shift) {
		DriverSchedule foundSchedule = null;
		for (DriverSchedule schedule : BtmsApplication.getBtms().getSchedule()) {
			if (schedule.getDriver().getId() == driverID && schedule.getAssignment().equals(routeAssignment) && schedule.getShift().equals(shift)) {
